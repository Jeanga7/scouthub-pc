...
export async function POST(
  ...
  try {
    ...
    const payload = workflowReviewRequestSchema.parse(await request.json());
    const actor = await requireProjectActor(request, currentRequestId);
    const useCases = createProjectUseCases();
    const project = await useCases.startProjectReview(
      mapWorkflowReviewRequest({ actor, payload, projectId, requestId: currentRequestId })
    );
