...
describe("PgProjectRepository", () => {
  beforeEach(async () => {
    ...
    try {
      await pool.query("TRUNCATE audit_event, project, role_assignment, account_invitation, account_person_link, account, person, organization RESTART IDENTITY CASCADE");
      await seedBase(pool);
    } finally {
      await pool.end();
    }
...
async function seedBase(pool: Pool): Promise<void> {
  ...
  await pool.query(
    `INSERT INTO account (id, external_identity_id, primary_email, status, email_verified_at)
     VALUES ($1, 'user_test', 'awa@example.test', 'ACTIVE', $2)`,
    [ids.account, now]
  );
  await pool.query(
